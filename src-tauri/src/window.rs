...
/// Resize the panel window to PANEL_BASE × scale, clamped to the
/// current monitor minus a small pad so it never opens larger than the
/// screen. Called by set_ui_scale on every change and by lib.rs setup
/// so a saved scale's window size is applied before the first show.
