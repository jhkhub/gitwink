...
/** App-wide event carrying a full settings snapshot — emitted by the
 *  Rust set_* commands so every window re-applies in lockstep. */
const SETTINGS_EVENT = "settings://changed";

let current: AppSettings = { ...DEFAULT_SETTINGS };

/** Mirror live settings into CSS custom properties on :root. The panel
 *  uses --ui-scale via `.panel { zoom: var(--ui-scale) }`; the diff
 *  uses calc(... * var(--ui-scale)) on `.sbs`. Each window calls this
 *  on load + on live change. */
...
/** The settings snapshot last loaded or received into this window. */
export function getCurrentSettings(): AppSettings {
  return current;
}

/** Load settings from the backend, apply them, and start listening for
 *  live changes (Rust emits settings://changed from the set_* commands).
 *  Called once per window mount, before first render. */
